...
type RichTextFieldProps<T extends FieldValues> = BaseProps<T> & {
    type: "richtext";
    minHeight?: string;
};
...
export function FormField<T extends FieldValues>(props: FormFieldProps<T>) {
    const { control, name, label, hint } = props;
                ...
                switch (props.type) {
                        ...
                        return (
                            <Field data-invalid={fieldState.invalid}>
                                ...
                                <Input
                                    ...
                                    value={props.allowEmpty ? field.value ?? "" : field.value ?? 0}
                                    onChange={(e) => {
                    ...
                    case "richtext": {
                        return (
                            <Field data-invalid={fieldState.invalid}>
                                <FieldLabel>{label}</FieldLabel>
                                <RichTextEditor value={field.value ?? ""} onChange={field.onChange} minHeight={props.minHeight} />
                                {hint && <p className="text-xs text-muted-foreground">{hint}</p>}
                                {fieldState.invalid && <FieldError errors={[fieldState.error]} />}
                            </Field>
                        );
                    ...
                    default: {
                        return (
                            <Field data-invalid={fieldState.invalid}>
                                <FieldLabel>{label}</FieldLabel>
                                <Input {...field} type={props.type ?? "text"} value={field.value ?? ""} placeholder={props.placeholder} />
                                {hint && <p className="text-xs text-muted-foreground">{hint}</p>}
                                {fieldState.invalid && <FieldError errors={[fieldState.error]} />}
                            </Field>
                        );
